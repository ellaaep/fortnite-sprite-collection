...
function spriteVisual(sprite, size = 'card') {
  const src = sprite.imageUrl || '';
  return `<div class="sprite-image-wrap ${size} image-local image-loaded">
    <img class="sprite-img" src="${src}" alt="${sprite.name}" loading="lazy" />
  </div>`;
}
...
async function hydrateSpriteImages(root = document) {
  // Local PNG build: no external image hydration needed.
}
